...
static def_DHelper(J_jal){
	decode_op_i(s,id_src1,s->isa.instr.u.imm31_12 <<1,true);
	if(id_src1->imm&0x100000){//如果是最高位为1就扩展符号位
	id_src1->imm|=0x80000000;
	id_src1->imm&=0xffefffff;
	}	
...
def_THelper(main) {
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 00100 11", I	  , arith);
